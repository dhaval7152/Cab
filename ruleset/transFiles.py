...
def main(argv):
	count=1
	ruleNum = 100
	inputFileName = "gpx_" + str(ruleNum) + "rules"
	outputFileName = str(ruleNum) + "rulesFile"
	with open(inputFileName, 'r') as f:
		with open(outputFileName, 'w') as w:
			for line in f.readlines():
				...
				s="priority={0},dl_type=0x0800,nw_proto=17,nw_src={1},nw_dst={2}{3}{4},actions=output:1".format(count,arr0,arr[1],temp1,temp2)
				count+=1
				#print(s)
				w.write(s+"\n")
	f.closed
